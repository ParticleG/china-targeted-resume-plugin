...
| Local OMP install/update/discovery | Phase 2 decision-log record plus help contract tests | Local link, package discovery, `/resume-help`, `/resume-status` outside project | OMP `17.3.7` loads seven commands, nine tools, Skill, and seven agents; help is model-free | Observed install/status plus focused help smoke; see `docs/migration-decision-log.md` | Observed local baseline |
