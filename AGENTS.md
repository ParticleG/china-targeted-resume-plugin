...
`SKILL.md` is the natural-language orchestration layer. `references/` owns detailed policy and data contracts. The Python package remains the executable source of truth.
...
| `references/` | Architecture, privacy, evidence, dossier, audit, output, and roadmap-handoff rules. |
...
No formatter, linter, static type checker, or corresponding command is configured. Do not invent Ruff, Black, Mypy, Pyright, line-length, or coverage-threshold requirements.
...
- `SKILL.md`: OMP Skill trigger and orchestration instructions.
...
- Required runtime: Linux with Python `>=3.14`.
- Package/environment manager: `uv`; keep `uv.lock` synchronized with dependency changes.
- Build backend: Hatchling (`hatchling.build`). The console script is `china-targeted-resume = china_targeted_resume.cli:main`.
...
- Runtime libraries are Pydantic, Jinja2, markdown-it-py, Playwright, and PyMuPDF. Dev dependencies are pytest and coverage.
- The wheel explicitly includes `assets/`; the sdist and curated `.skill` archive include broader schemas, references, scripts, and Skill metadata. Do not assume every repository file exists in a wheel install.
...
- Framework: pytest 8.x. Full suite: `uv run pytest -q`.
- Tests use module-level `test_<observable_contract>` functions in `tests/test_<area>.py`; no test classes, custom markers, pytest configuration, or CI workflow are defined.
...
- Test observable boundaries: canonical model validation, traversal/symlink rejection, source immutability, evidence/provenance invariants, private modes, non-overwriting output, artifact schemas, and CLI stdout/stderr contracts.
